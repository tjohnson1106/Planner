...
function printRunInstructions(projectDir, projectName) {
  ...
  const xcodeProjectPath = `${_path().default.resolve(
    projectDir,
    'ios',
    projectName
  )}.xcodeproj`;

  const relativeXcodeProjectPath = _path().default.relative(
    process.cwd(),
    xcodeProjectPath
  );
  ...
  _logger.default.log(`
  ${_chalk().default.cyan(
    `Run instructions for ${_chalk().default.bold('iOS')}`
  )}:
  ...
  ${_chalk().default.green(
    `Run instructions for ${_chalk().default.bold('Android')}`
  )}:
