...
var _findMarkedLinesInPodfile = _interopRequireDefault(
  require('./findMarkedLinesInPodfile')
);
...
function registerNativeModulePods(name, dependencyConfig, iOSProject) {
  ...
  (0, _addPodEntry.default)(
    podLines,
    linesToAddEntry,
    dependencyConfig.podspec,
    name
  );
...
function getLinesToAddEntry(podLines, { projectName }) {
  const linesToAddPodWithMarker = (0, _findMarkedLinesInPodfile.default)(
    podLines
  );
  ...
  const firstTargetLined = (0, _findPodTargetLine.default)(
    podLines,
    projectName
  );
