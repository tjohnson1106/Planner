...
const getOuterDirectory = directories =>
  directories.reduce((topDir, currentDir) => {
    ...
    if (
      ...
      (0, _lodash().last)(currentFolders) !==
        (0, _lodash().last)(topMostFolders)
    ) {
...
function getHeaderSearchPath(sourceDir, headers) {
  const directories = (0, _lodash().union)(
    headers.map(_path().default.dirname)
  );
  return directories.length === 1
    ? `"$(SRCROOT)${_path().default.sep}${_path().default.relative(
        sourceDir,
        directories[0]
      )}"`
    : `"$(SRCROOT)${_path().default.sep}${_path().default.relative(
