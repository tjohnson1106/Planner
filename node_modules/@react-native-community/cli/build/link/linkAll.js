...
var _getAssets = _interopRequireDefault(require('../core/getAssets'));
...
const dedupeAssets = assets =>
  (0, _lodash().uniqBy)(assets, asset => _path().default.basename(asset));
...
function linkAll(context, platforms, project) {
  ...
  const depenendenciesConfig = dependencies.map(dependnecy =>
    (0, _getDependencyConfig.default)(context, platforms, dependnecy)
  );
  ...
  const tasks = (0, _lodash().flatten)(
    depenendenciesConfig.map(config => [
      () =>
        (0, _promisify.default)(
          config.commands.prelink || _commandStub.default
        ),
      () => (0, _linkDependency.default)(platforms, project, config),
      () =>
        (0, _promisify.default)(
          config.commands.postlink || _commandStub.default
        ),
      () => (0, _linkAssets.default)(platforms, project, assets)
    ])
  );
