...
var _getPackageConfiguration = _interopRequireDefault(
  require('./getPackageConfiguration')
);
...
const findAssetsInFolder = folder =>
  _glob().default.sync(_path().default.join(folder, '**'), {
    nodir: true
  });
/**
...
function findAssets(folder, assets) {
  return (assets || [])
    .map(asset => _path().default.join(folder, asset))
    .reduce((acc, assetPath) => acc.concat(findAssetsInFolder(assetPath)), []);
}
/**
