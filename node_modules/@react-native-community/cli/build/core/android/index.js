...
var _findAndroidAppFolder = _interopRequireDefault(
  require('./findAndroidAppFolder')
);

var _findManifest = _interopRequireDefault(require('./findManifest'));

var _findPackageClassName = _interopRequireDefault(
  require('./findPackageClassName')
);

var _readManifest = _interopRequireDefault(require('./readManifest'));

var _android = _interopRequireDefault(require('../../link/android'));
 ...
 * @format
 */
const linkConfig = _android.default;
...
function projectConfig(folder, userConfig = {}) {
  const src =
    userConfig.sourceDir || (0, _findAndroidAppFolder.default)(folder);
  ...
  const manifestPath = userConfig.manifestPath
    ? _path().default.join(sourceDir, userConfig.manifestPath)
    : (0, _findManifest.default)(sourceDir);
  ...
  const packageFolder =
    userConfig.packageFolder || packageName.replace(/\./g, _path().default.sep);

  const mainFilePath = _path().default.join(
    sourceDir,
    userConfig.mainFilePath ||
      `src/main/java/${packageFolder}/MainApplication.java`
  );

  const stringsPath = _path().default.join(
    sourceDir,
    userConfig.stringsPath || 'src/main/res/values/strings.xml'
  );

  const settingsGradlePath = _path().default.join(
    folder,
    'android',
    userConfig.settingsGradlePath || 'settings.gradle'
  );

  const assetsPath = _path().default.join(
    sourceDir,
    userConfig.assetsPath || 'src/main/assets'
  );

  const buildGradlePath = _path().default.join(
    sourceDir,
