  ...
  return deps.reduce((acc, pkg) => {
    ...
    if (isReactNativePlugin(pkg)) {
      const pkgJson = readPackage(
        _path().default.join(folder, 'node_modules', pkg)
      );
...
function findPlugins(folder) {
  ...
  return {
    ...
    haste: {
      platforms: (0, _lodash().uniq)(
        (0, _lodash().flatten)(plugin.haste.platforms)
      ),
      providesModuleNodeModules: (0, _lodash().uniq)(
        (0, _lodash().flatten)(plugin.haste.providesModuleNodeModules)
      )
    }
  };
}
