  ...
  const symlinks = moduleFolders.reduce((links, folderName) => {
    ...
    if (folderName.startsWith('@')) {
      ...
      maybeSymlinkPaths.push(
        ...scopedModuleFolders.map(name =>
          _path().default.join(folderPath, name)
        )
      );
    } else {
  ...
  const nestedSymlinks = symlinks.reduce(
    (links, symlinkPath) =>
      links.concat(
        ...
        findModuleSymlinks(_path().default.join(symlinkPath, 'node_modules'), [
          ...ignoredPaths,
          ...symlinks
        ])
      ),
...
function resolveSymlinkPaths(maybeSymlinkPaths, ignoredPaths) {
  return maybeSymlinkPaths.reduce((links, maybeSymlinkPath) => {
    if (
      _fs()
        .default.lstatSync(maybeSymlinkPath)
        .isSymbolicLink()
    ) {
      const resolved = _path().default.resolve(
        _path().default.dirname(maybeSymlinkPath),
        _fs().default.readlinkSync(maybeSymlinkPath)
      );

      if (
        ignoredPaths.indexOf(resolved) === -1 &&
