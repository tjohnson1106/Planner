...
var _parseIOSDevicesList = _interopRequireDefault(
  require('./parseIOSDevicesList')
);

var _findMatchingSimulator = _interopRequireDefault(
  require('./findMatchingSimulator')
);
...
function runIOS(_, ctx, args) {
  ...
  const xcodeProject = (0, _findXcodeProject.default)(
    _fs().default.readdirSync('.')
  );
  ...
  const inferredSchemeName = _path().default.basename(
    xcodeProject.name,
    _path().default.extname(xcodeProject.name)
  );
  ...
  if (args.device) {
    ...
    if (devices && devices.length > 0) {
      ...
      _logger.default.error(`Could not find device with the name: "${
        args.device
      }".
...
async function runOnSimulator(xcodeProject, args, scheme) {
  ...
  const selectedSimulator = (0, _findMatchingSimulator.default)(
    simulators,
    args.simulator
  );
  ...
  _child_process().default.spawnSync(
    'xcrun',
  ...
  const bundleID = _child_process()
    .default.execFileSync(
      ...
      [
        ...
        _path().default.join(appPath, 'Info.plist')
      ],
      {
        encoding: 'utf8'
      }
...
async function runOnDevice(
  ...
  const iosDeployOutput = _child_process().default.spawnSync(
    'ios-deploy',
