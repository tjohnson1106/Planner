...
var _isPackagerRunning = _interopRequireDefault(
  require('../util/isPackagerRunning')
);
...
function checkAndroid(root) {
  return _fs().default.existsSync(
    _path().default.join(root, 'android/gradlew')
  );
}
/**
...
function buildAndRun(args) {
  ...
  const packageName = _fs()
    .default.readFileSync(`${appFolder}/src/main/AndroidManifest.xml`, 'utf8') // $FlowFixMe
    .match(/package="(.+?)"/)[1];
  ...
  if (args.deviceId) {
    ...
    return (0, _runOnAllDevices.default)(
      args,
      cmd,
      packageNameWithSuffix,
      packageName,
...
function startServerInNewWindow(
  ...
  const launchPackagerScript = _path().default.join(
    reactNativePath,
    `scripts/${scriptFile}`
  );
  /**
  ...
  const packagerEnvFile = _path().default.join(scriptsDir, packagerEnvFilename); // eslint-disable-next-line flowtype/no-weak-types
  ...
  if (process.platform === 'darwin') {
    if (terminal) {
      return (0, _child_process().spawnSync)(
        'open',
